...
\section{Deep Learning - Bengio}
	...
	% We want to figure out if the parameter (weights) we have learned so far are correct (which they must likely aren't). If they are incorrect we want to modify them so they diminish the error of our network.
	% First step, we compute the error of our network with forward propagation of the input.

	% \begin{algorithm}[H]
	% 	\KwData{the inputs x are given to the algorithm}
	% 	\KwResult{returns the error given the weight }
	% 	\BlankLine
	% 	\emph{\% Initialize the algorithm, $U_0$ is the inputs}\;
	% 	$U_0 \leftarrow x$\;
	% 	\emph{\% forward propagate input signal to output layer}\;
	% 	\For{$i \leftarrow 1$ \KwTo $N$}{
	% 		$U_i \leftarrow f_i(U_{i-1})$\;
	% 	}
	...
	% We take a little step down the error modifying the weights on the network.

	% \begin{algorithm}[H]
	% 	\KwData{the inputs x are given to the algorithm}
	% 	\KwResult{returns a set of partial derivatives for every hidden nodes}
	% 	\BlankLine
	% 	\emph{\% Initialize the algorithm}\;
	% 	$\frac{\partial U_N}{\partial U_N} \leftarrow 1$\;
	% 	\emph{\% back propagate error throughout the network}\;
	% 	\For{$j \leftarrow N-1$ \KwTo $1$}{
	% 		$\frac{\partial U_N}{\partial U_j} \leftarrow 
	% 			\frac{\partial U_N}{\partial U_{j+1}} \frac{\partial f_j(U_{j+1}}{\partial U_{j+1}}$\;
	% 	}
	...
	% We first compute the forward message, in other terms, we compute the output of our actual network given the actual parameters $\theta$. It is straight forward, 


	% To compute the gradient of the cost-function in the network, 
	...
	\begin{figure}
		...
		\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep]
			...
			\node[annot] at (-1,\layersep*4) {Hidden layer i};
			\node[annot] at (-1,\layersep*3) {Hidden layer j};
